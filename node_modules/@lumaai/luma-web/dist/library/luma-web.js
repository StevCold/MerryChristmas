...
`),additionalUniforms:{inv_proj:["mat4",{value:new w.Matrix4}],...A?.vertexShaderHooks?.additionalUniforms},getSplatColor:A?.vertexShaderHooks?.getSplatColor,onMainEnd:`
					() {
						...
						${A?.vertexShaderHooks?.onMainEnd?"_three_onMainEnd()":""};
					}
				`,getSplatTransform:A?.vertexShaderHooks?.getSplatTransform},fragmentShaderHooks:{additionalGlobals:[Q.ShaderChunk.fog_pars_fragment,Q.ShaderChunk.clipping_planes_pars_fragment,A?.fragmentShaderHooks?.additionalGlobals,A?.fragmentShaderHooks?.getFragmentColor?`vec4 _three_getFragmentColor${A.fragmentShaderHooks.getFragmentColor}`:void 0].join(`
`),additionalUniforms:{...A?.fragmentShaderHooks?.additionalUniforms},getFragmentColor:`
					(vec4 fragColor) {
						...
						#undef gl_FragColor
						return ${A?.fragmentShaderHooks?.getFragmentColor?"_three_getFragmentColor(fragColor)":"fragColor"};
					}
				`}}:A,I=d(g),B=D?{...Q.UniformsLib.fog}:{},C=g?.vertexShaderHooks,E=g?.fragmentShaderHooks;if(C?.additionalUniforms)for(let s in C.additionalUniforms)B[s]=C?.additionalUniforms[s][1];if(E?.additionalUniforms)for(let s in E.additionalUniforms)B[s]=E?.additionalUniforms[s][1];let i=/#version\s+([^\n]+)\n/,e=I.fragmentShader.match(i),t=I.vertexShader.match(i),a=I.vertexShader.replace(i,""),o=I.fragmentShader.replace(i,"");return{glslVersion:(e?.[1]||t?.[1])??"300 es",vertexShader:a,fragmentShader:o,additionalUniforms:B}}function f(D){"use strict";let A=(D&31744)>>10,g=D&1023;return(D>>15?-1:1)*(A?A===31?g?NaN:1/0:Math.pow(2,A-15)*(1+g/1024):6103515625e-14*(g/1024))}return EA(hA);})();
