...
`),additionalUniforms:{inv_proj:["mat4",{value:new h}],...A?.vertexShaderHooks?.additionalUniforms},getSplatColor:A?.vertexShaderHooks?.getSplatColor,onMainEnd:`
				...
				`,getSplatTransform:A?.vertexShaderHooks?.getSplatTransform},fragmentShaderHooks:{additionalGlobals:[u.fog_pars_fragment,u.clipping_planes_pars_fragment,A?.fragmentShaderHooks?.additionalGlobals,A?.fragmentShaderHooks?.getFragmentColor?`vec4 _three_getFragmentColor${A.fragmentShaderHooks.getFragmentColor}`:void 0].join(`
`),additionalUniforms:{...A?.fragmentShaderHooks?.additionalUniforms},getFragmentColor:`
				...
				`}}:A,I=x(g),B=a?{...xA.fog}:{},C=g?.vertexShaderHooks,Q=g?.fragmentShaderHooks;if(C?.additionalUniforms)for(let D in C.additionalUniforms)B[D]=C?.additionalUniforms[D][1];if(Q?.additionalUniforms)for(let D in Q.additionalUniforms)B[D]=Q?.additionalUniforms[D][1];let E=/#version\s+([^\n]+)\n/,o=I.fragmentShader.match(E),e=I.vertexShader.match(E),t=I.vertexShader.replace(E,""),i=I.fragmentShader.replace(E,"");return{glslVersion:(o?.[1]||e?.[1])??"300 es",vertexShader:t,fragmentShader:i,additionalUniforms:B}}function j(a){"use strict";let A=(a&31744)>>10,g=a&1023;return(a>>15?-1:1)*(A?A===31?g?NaN:1/0:Math.pow(2,A-15)*(1+g/1024):6103515625e-14*(g/1024))}export{l as LumaSplatsLoader,_ as LumaSplatsSemantics,EA as LumaSplatsThree,K as LumaSplatsWebGL,x as getLumaShaderCode};
